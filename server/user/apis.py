...
from user.utils import get_password_hash, verify_password, create_access_token, get_current_user_model
...
async def post_register_user(user: UserLoginPydantic):
    ...
    if not user_obj:
        user_obj = await User.create(username=user.username, password=password_hash, avatar=DEFAULT_AVATAR)
...
async def get_me_detail(user: User = Depends(get_current_user_model)):
    return UserInfoPydantic.from_orm(user)
